    ...
    <main className="mx-auto max-w-4xl px-6 pb-24 pt-10">
      ...
      <header className="mt-8 grid grid-cols-[auto_1fr] items-end gap-x-10 border-b border-stone-900 pb-10">
        <div>
          ...
          <p className="mt-1 font-serif text-7xl leading-none text-stone-100 tabular-nums md:text-8xl">
            {fmtDayNum(video.recorded_at)}
          </p>
          <p className="mt-2 font-mono text-[10px] tracking-[0.18em] text-stone-600">
            {fmtYear(video.recorded_at)} · {fmtTime(video.recorded_at)}
          </p>
        </div>
        <div className="pb-2">
          {topTopic && (
            <p className="small-caps text-[10px] text-stone-500">
          ...
          <div className="mt-4 flex flex-wrap items-baseline gap-x-5 gap-y-1 text-xs text-stone-500 tabular-nums">
      ...
      {segments.length > 0 && (
        <section className="mt-16">
          <p className="small-caps text-xs text-stone-500">The lesson, in sequence</p>
            ...
            {segments.map((seg) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <span className="absolute left-0 top-6 bottom-6 w-[2px] bg-moss-500" />
                  )}
                  {/* Timestamp button */}
                  <div className="pt-1">
                    <button
                    ...
                    {seg.dominant_speaker && (
                      <p className="mt-2 small-caps text-[10px] text-stone-600">
                        {seg.dominant_speaker}
                      </p>
                    )}
                  </div>
                      ...
                      <div className="mt-4 flex flex-wrap items-baseline gap-x-4 gap-y-1.5">
                        {segTopics.map((m) => (
                          <a
                            ...
                            className="font-serif italic text-sm text-stone-400 transition-colors hover:text-moss-300"
                          >
                            {m.topic_name}
                          </a>
                        ))}
                        {segDrills.map((m) => (
                          <span
                            key={`d-${m.id}`}
                            className="small-caps text-[11px] text-moss-300"
                          >
                            drill · {m.drill_name}
                          </span>
                        ))}
      ...
      {topicMentions.filter((m) => m.segment_id == null).length > 0 && (
        <section className="mt-16">
          <p className="small-caps text-xs text-stone-500">Other topics noted</p>
          <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 font-serif italic text-stone-400">
            {topicMentions
      ...
      {transcript && (
        <section className="mt-16 border-t border-stone-900 pt-8">
          <button
            ...
            className="small-caps flex items-baseline gap-2 text-xs text-stone-500 transition-colors hover:text-stone-200"
          >
