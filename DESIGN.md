...
| 2026-05-19 | Auto-symlink ingested videos into `src-demo/public/video/` | After the ingest, the lesson detail page rendered video elements with `src="/video/<name>"` but the files were absent from `public/video/` — Next.js returned 404 and the video players showed broken. The existing 19 demo videos had been manually copied to `public/video/` during initial setup; the 5 new May 2025 files lived only at `Recordings/ScreenRecording_*.mov` and weren't reachable. Compounding bug: the demo worktree at port 3300 had its own empty `public/video/` directory, so even the existing 19 lessons couldn't play there. Fix: (1) symlinked `golf-coach-demo/src-demo/public/video` → master's directory so the demo worktree inherits the served files. (2) added `_link_into_public_video()` helper in `triage.py` that symlinks each newly-classified video file into `public/video/` so future ingests don't repeat the bug. Uses symlinks not copies — saves disk on multi-GB recordings. |
| 2026-05-19 | Topic subcategory round 2 — 17 more topics classified, 2 more dupes merged | After the initial sub-category pass left 34 topics with `subcategory = NULL`, a second pass classified 17 of them into existing or new buckets and surfaced two more duplicate clusters that escaped the first cleanup. New subcategories: **Impact** (Attack Angle, Impact Position, Dynamic Loft, Launch Angle — 17 mentions) and **Shot Shape** (Draw, Ball Flight, Ball flight shape, Starting Line — 9 mentions). Existing buckets expanded: Backswing += Takeaway; Club Path += Over-the-Top + Club Exit + Club Exit Direction; Shaft += Shallowing; Low Point += Arc Bottom / Low Point; Wrist += Hand Path + Early Release; Setup += Spine Tilt. Merged duplicates: `Angle of Attack` → `Attack Angle`, `Shaft Shallowing` → `Shallowing`. Result: 92 → 90 topics, 71 of 90 now have a subcategory (was 55 of 92). 19 topics remain standalone — these are the legitimate one-offs (Swing Plane, Lag, Video Analysis, Head Position, etc.) that don't warrant a parent bucket. Migration at `src-demo/db/migrations/2026-05-19-topic-subcategory-round-2.sql`. |
| 2026-05-19 | Backfilled stale `file_hash` values on the 19 original embedded videos | When `triage.py` first ran on the Trackman folder during the mass-ingest experiment, it re-applied ffmpeg faststart to the already-faststart-processed files; the byte-level non-determinism produced new hashes that didn't match the DB's stored values. The previous fix (reordering dup-check before faststart in `triage.py`) prevents new dups going forward but didn't repair the 19 stale hash rows. A one-shot Python script using `classify._hash_file()` (the same function `triage.py` uses) recomputed each video's current sha256 and updated the row. Verified idempotent — the next `triage.py` run will correctly mark these files as duplicates. Data-only fix, no schema or code changes; documented here so the rationale survives. |
| 2026-05-19 | Streaming answers on `/ask` — token-by-token rendering via Server-Sent Events | The non-streaming `client.messages.create` call meant users stared at "Searching the journal…" for 3-6 seconds with no feedback before the whole answer popped in. Now `askWithRagStreaming()` in `lib/rag.ts` wraps `client.messages.stream()` and returns a `ReadableStream<Uint8Array>` of SSE-encoded events: `sources` (sent first so the citation list renders while the model is warming up), `delta` (each token chunk), `done` (final token counts), `error` (any mid-stream failure). The API route at `/api/ask` returns this stream with `Content-Type: text/event-stream`. The `/ask` page consumes via `response.body.getReader()` + a `TextDecoder({stream:true})` to handle multi-byte UTF-8 across reads, plus a line buffer that holds incomplete SSE fragments between reads. Citation renderer is partial-text-safe — `[1` mid-stream renders as plain text until the `]` arrives. |
| 2026-05-19 | Drag-and-drop upload page at `/upload` for ad-hoc lesson ingest | Previously, adding a new lesson required running `triage.py` + `process.py` from the command line. The new `/upload` page accepts a single .mp4 or .mov via drag-drop (or click-to-pick), POSTs to `/api/upload`, saves to `Recordings/_uploads/` (with timestamp suffix on filename conflict), then spawns the Python pipeline as child processes — first `triage.py --limit 1` to classify + insert + auto-symlink, then `process.py --video-id N` to advance through transcribe → analyze → embed. The API streams SSE events back: `stage` for transitions, `log` for each subprocess stdout line, `done` with `session_id`/`video_id` on success, `skipped` with reason if the speech-detection rejects it as a silent radar clip. Server-side validation: extension whitelist (mp4/mov), MIME whitelist (video/mp4, video/quicktime), 1 GB cap — all checked before any disk write. UI matches the editorial-quiet aesthetic: dashed border drop zone with moss-green hover, stage list with mono elapsed timers + ✓ marks on completion, "→ Open the lesson" affordance pointing at `/lessons/{session_id}?v={video_id}`. SiteNav gained a fourth link. Built via parallel subagents (Agent A: streaming, Agent C: upload) with worktree isolation. |
